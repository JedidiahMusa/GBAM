...
const ValueCard: React.FC<ValueCardProps> = ({ title, description, icon }) => (
  <div
    className="bg-white rounded-xl shadow-lg p-6 transition-all duration-300 hover:shadow-xl hover:translate-y-[-5px]"
    data-aos="fade-up"
  >
    <Box className="text-gbam-primary mb-4 text-2xl">{icon}</Box>
    <Heading
      as="h3"
      mb={2}
      className="!text-xl font-bold text-gbam-dark"
    >
      {title}
    </Heading>
